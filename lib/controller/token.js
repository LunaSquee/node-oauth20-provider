    ...
    async.waterfall([
        ...
        function(cb) {
            if (req.body.client_id && req.body.client_secret) {
                ...
                if (!pieces || pieces.length !== 2)
                    return cb(new error.invalidRequest('Authorization header is invalid'));
                ...
                if (!pieces || pieces.length !== 2)
                    return cb(new error.invalidRequest('Authorization header is incorrectly formatted'));
        ...
        function(cb) {
            if (!req.body.grant_type)
                return cb(new error.invalidRequest('Body does not contain grant_type parameter'));
