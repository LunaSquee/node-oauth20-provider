    ...
    async.waterfall([
        ...
        function(cb) {
            if (!req.query.redirect_uri)
                return cb(new error.invalidRequest('redirect_uri is mandatory for authorization'));

            redirectUri = req.query.redirect_uri;
            req.oauth2.logger.debug('redirect_uri parsed: ', redirectUri);
            cb();
        },
        // Check client credentials
        function(cb) {
            if (!req.query.client_id)
                return cb(new error.invalidRequest('client_id is mandatory for authorization'));

            // Check for client_secret (prevent from passing it)
            if (req.query.client_secret)
                return cb(new error.invalidRequest('client_secret should not be passed by public clients'));

            clientId = req.query.client_id;
            req.oauth2.logger.debug('client_id parsed: ', clientId);
            cb();
        },
        // Check response type parameter
        function(cb) {
            if (!req.query.response_type)
                return cb(new error.invalidRequest('response_type parameter is mandatory for authorization'));
        ...
        function(cb) {
            ...
            if (!user)
                cb(new error.invalidRequest('Failed to fetch currently logged in user'));
            else {
